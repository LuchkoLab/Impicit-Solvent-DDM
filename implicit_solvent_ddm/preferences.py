...
def as_python_object(dct):
    ...
    if "<type>" in dct:
        ...
        if dct["<type>"] == "int":
            return int
        raise TypeError(
            "'" + dct["<type>"] + "' is not a support type for preference files"
        )
    return dct
    ...
    def addFile(self, file):
        ...
        if "include" in prefs.keys():
            if isinstance(prefs["include"], str):
                prefs["include"] = os.path.join(os.path.dirname(file), prefs["include"])
        ...
        for key, value in self.nested_dict_iter(prefs):
            if "file" in value:
                ...
                value["file"] = os.path.join(os.path.dirname(file), value["file"])
                self.log.debug(
                    "Setting file '" + relative_path + "' to : " + value["file"]
                )
